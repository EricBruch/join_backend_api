...
def create_assigned_users(assignedUsers, task):
    for aU in assignedUsers:
        newAssignedUser = TaskAssignedUser.objects.create(
            user=aU.get("user"), task=task, created_at=now_or_date(aU.get("created_at"))
        )
...
def getMappedTaskInstance(instance, validated_data):
    ...
    instance.created_at = validated_data.pop("created_at", instance.created_at)
